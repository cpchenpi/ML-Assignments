...
`sklearn` 提供的这些算法关键参数不多，可以直接使用网格法选择参数。对已经调节参数后的模型进行比较，随机划分数据集 5 次，比较它们的平均准确率。一次运行得到的数据如下：


#figure(image("image/compare.png", width: 50%), caption: "几个经典模型的平均准确率比较")

随机森林算法的一般表现更好，这符合我们的期望。

== 结果可视化

使用 t-SNE 降维算法，将测试集降至二维可视化，得到结果如下图：

#figure(image("image/tsne.png", width: 70%), caption: "使用 t-SNE 算法降维后的数据散点图")

其中预测错误的样本为黑色，预测正确的机器人样本为绿色，预测正确的人类样本为黄色。可以看到至少对降维至二维的数据，可分性不是特别强，大部分分类错误的样本在分类边界上。
